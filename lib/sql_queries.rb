...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT * FROM (SELECT title, SUM(amount) - funding_goal AS result FROM projects, pledges WHERE projects.id = pledges.project_id GROUP BY title ORDER BY title) WHERE result >= 0"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
  "SELECT name, SUM(amount) FROM users, pledges WHERE users.id = pledges.user_id GROUP BY name ORDER BY SUM(amount), name"
end
